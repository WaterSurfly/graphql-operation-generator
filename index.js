...
const arr = [];
const getTypeDefs = (schemaFilePath) => {
    try {    
       fs.readdirSync(schemaFilePath,  { withFileTypes: true })
          .forEach((file) => {
             const path = `${schemaFilePath}/${file.name}`;
             if (file.isDirectory()) {
                getTypeDefs(path);
             } else {
                arr.push(fs.readFileSync(path, 'utf-8'));
             }
          });
...
const types = getTypeDefs(schemaFilePath);
const source = new Source(types.toString());
const gqlSchema = buildSchema(source);

del.sync(destDirPath);
path.resolve(destDirPath)
    .split(path.sep)
    .reduce((before, cur) => {
    ...
    if (curType.getFields) {
        ...
        childQuery = childKeys
            .filter((fieldName) => {
                ...
                return (
                    includeDeprecatedFields || !fieldSchema.deprecationReason
                );
            })
            .map(
                (cur) =>
